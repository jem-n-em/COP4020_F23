...
public class Parser implements IParser {
	...
	//Verifies presence of terminal symbols that are not important to the abstract syntax of the production
	public boolean matchSingle(Kind kind) throws SyntaxException, LexicalException{
		if(!match(kind)){
			throw new SyntaxException(currToken.sourceLocation(), kind + " expected but was " + currToken.kind());
		}
		return true;
	}
	...
	public AST parse() throws PLCCompilerException {
		AST e = program();
		if(currToken.kind() != Kind.EOF){
			throw new SyntaxException(currToken.sourceLocation(), "No code allowed past block close!: " + currToken.kind());
		}
		return e;
	}
